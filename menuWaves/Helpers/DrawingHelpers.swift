...
class DrawingHelpers {
    ...
    static func drawWave(in rect: NSRect, waveAnimator: WaveAnimator, chargingEffectProgress: CGFloat) {
        ...
        // Motion blur cho sóng: vẽ nhiều lớp với độ mờ dần
        let blurLayers = 3
        ...
        for layer in 0..<blurLayers {
            ...
            for (i, x) in xRange.enumerated() {
                ...
                let y = midY + amp * sin(frequency * (x / rect.width * 2 * .pi) + waveAnimator.phase) + layerOffset
                let pt = NSPoint(x: x, y: y)
                
                if let prev = prevPt {
                    let frac = CGFloat(i) / CGFloat(xRange.count-1)
                    let positionRatio = x / rect.width
                    ...
                    if chargingEffectProgress > 0 {
                        ...
                        if positionRatio >= waveStart && positionRatio <= waveEnd {
                            ...
                            if localPosition <= 0.5 {
                                greenIntensity = localPosition * 2
                            } else {
                                greenIntensity = (1.0 - localPosition) * 2
                            }
                        } else {
                            greenIntensity = 0.0
                        }
                    ...
                    let blurColor = finalColor.withAlphaComponent(layerAlpha)
                    blurColor.setStroke()
        ...
        if waveAnimator.drawProgress >= 1.0 {
            ...
            for i in 0..<sparkleCount {
                ...
                let sparkleAlpha = 0.7 + 0.3 * sin(waveAnimator.phase * 1.5 + CGFloat(i) * 2)
    ...
    private static func drawStar(at point: NSPoint, size: CGFloat) {
